...
public class User : IValidatableObject
{
    ...
    [StringLength(120, MinimumLength = 2, ErrorMessage = "Full name must be 2-120 characters.")]
    ...
    public static readonly string[] AllowedRoles = ["Prosumer", "Consumer", "Producer", "Admin", "Grid Operator"];

    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        if (!string.IsNullOrEmpty(FullName) && string.IsNullOrWhiteSpace(FullName))
            yield return new ValidationResult("Full name cannot be blank.", [nameof(FullName)]);
